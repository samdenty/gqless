...
using them, that's why you have to specify a special option `nonSerializableVariables` in your [`resolved`](/client/fetching-data#resolved)s
...
Since the images are not serializable, GQless might be confused while
using them, that's why you have to specify a special option `nonSerializableVariables` in your [`useMutation`](/react/mutations)s
