...
That allows preventing some possible cascading, or simply to make sure to get all the fields requested in the same query,
specially while using conditionals on possible nulls in your GraphQL Schema.

For that use case we offer a very useful function called `prepass`, in which you can give it an object or array, and given some paths you give it, it will traverse them,
...
- You **don't** have to specify an array index, since if it finds an array, it pass-through the **first non-null** value of it
