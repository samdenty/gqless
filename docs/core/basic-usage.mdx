...
`inlineResolved` is similar to [resolved](#resolved), you give it a pre-defined function, but it **won't isolate** it's requests, instead, it will use the `scheduler` _indirectly_, and asynchronously merge your concurrent requests,
...
You can also specify some options in the second argument, like forcing refetch and/or intercepting existing cache while refetching, check [**InlineResolveOptions**](/core/api/interfaces/inlineresolveoptions) for more information.
