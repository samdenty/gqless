...
- The [`prepass`](/core/helpers#prepass) helper.
...
- Suspense support
- Partial [Fetch policy](/react/api-reference#fetch-policy) support (`'cache-first'`, `'cache-and-network'` or `'network-only'`)
...
- [Core helpers](/core/helpers) included in third parameter of main function.
...
- Lifecycle functions `onCompleted` & `onError`
- Suspense support
- Partial [Fetch policy](/react/api-reference#fetch-policy) support (`'network-only'`, `'cache-and-network'` or `'no-cache'`)
