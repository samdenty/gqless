...
First of all, **We suggest NOT using the `graphql HOC`** for `Non-Suspense usage`, instead, do [useQuery](/react/query#usequery), mainly because it returns an extra reserved `gqlessState` object, in which you can check easily if the data you requested is being fetched for the first time:
...
function Example() {
  ...
  if (gqlessState.isLoading) return <p>Loading...</p>;
