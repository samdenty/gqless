...
your schema are now just types, rather than typed instances of classes. This flattens things quite a bit, and means
even on very large schemas we've seen no performance issues.
...
v2 adds [caching strategies](/core/persistence) that allow your queries to automatically resolve previously fetched data. It supports the `stale-while-revalidate` pattern
...
Built in [noramlized caching](/core/config#normalization) works as well, which will recognize the same object across different types of queries.

## Server-side Rendering

v2 adds Server-Side rendering (**SSR**) support first-class.

## React Hooks

v2 removes the need to use the higher order component, and lets you just use a [simple useQuery hook](/react/query) for React.
...
v2 adds many new configuration options, with highlights including `retry` for retrying failed queries automatically.
